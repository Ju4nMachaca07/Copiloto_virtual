...
data class RouteSegment(
    ...
    val warningDistance: Double = 500.0
)
...
class GeofenceManager {
    ...
    private val announcementCooldown = 5000L
    ...
    fun generateSegments(
        ...
        for (i in 0 until waypoints.size - 1) {
            ...
            val bearing = MapUtils.calculateBearing(start, end)
            val direction = MapUtils.bearingToDirection(bearing)
            val distance = MapUtils.calculateDistance(start, end)
            ...
            val speedLimit = SpeedLimitZones.getZoneAt(end)?.speedLimit ?: when {
                distance < 5000 -> 40
                distance < 20000 -> 60
                else -> 80
            }
    ...
    fun checkGeofence(
        ...
        // Alertas de proximidad
            ...
            for (announcementDistance in announcementDistances) {
                if (distanceToEnd <= announcementDistance && !announcedDistances.contains(announcementDistance)) {
                    val message = when {
        ...
        if (currentTime - lastAnnouncementTime > announcementCooldown * 2) {
            currentSegment.speedLimit?.let { limit ->
                ...
                if (speedKmh > limit + 10) {
                    onEnterSegment(
                        currentSegment,
                        "Atención. Reducir velocidad. Límite ${limit} kilómetros por hora. Velocidad actual ${speedKmh} kilómetros por hora"
                    )
                    lastAnnouncementTime = currentTime
                }
            }
    ...
    fun getProgress(totalSegments: Int): Float {
        ...
        return currentSegmentIndex.toFloat() / totalSegments.toFloat()
    }
